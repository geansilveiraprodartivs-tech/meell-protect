...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "GET") {
    return new Response(JSON.stringify({ ok: true, service: "delivery-download", version: "1.0.0" }), {
      status: 200,
  ...
  try {
    ...
    if (protectionMode === "none" || !canWatermark) {
      ...
      // Increment counters server-side
      await supabase.from("deliveries").update({
        download_count: delivery.download_count + 1,
        last_downloaded_at: new Date().toISOString(),
      }).eq("id", delivery.id);

      await supabase.from("protected_files").update({
    ...
    // 9. Increment counters and record event
    await supabase.from("deliveries").update({
      download_count: delivery.download_count + 1,
      last_downloaded_at: new Date().toISOString(),
    }).eq("id", delivery.id);
